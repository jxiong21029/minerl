...
from minerl.herobraine.wrappers.util import (
    union_spaces,
    flatten_spaces,
    intersect_space,
)
...
class Vectorized(EnvWrapper):
    ...
    def __init__(self, env_to_wrap: EnvSpec, common_envs=None):
        assert (
            env_to_wrap.is_single_agent
        ), "Vectorized (currently) only supports single agents environments."
        self.env_to_wrap = env_to_wrap
        self.common_envs = (
            [env_to_wrap]
            if common_envs is None or len(common_envs) == 0
            else common_envs
        )

        # Gather all of the handlers for the common_env
        self.common_actions = reduce(
            union_spaces, [env.actionables for env in self.common_envs]
        )
        self.common_action_space = spaces.Dict(
            {hdl.to_string(): hdl.space for hdl in self.common_actions}
        )
        self.common_observations = reduce(
            union_spaces, [env.observables for env in self.common_envs]
        )
        ...
        obs_list.append(
            (
                ...
                spaces.Box(
                    low=0.0,
                    high=1.0,
                    shape=[self.observation_vector_len],
                    dtype=np.float32,
                ),
            )
        )
