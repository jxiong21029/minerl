...
class Obfuscated(EnvWrapper):
    def __init__(self, env_to_wrap: Vectorized, obfuscator_dir, name=""):
        ...
        assert isinstance(
            env_to_wrap, Vectorized
        ), "Obfuscated env wrappers only supported for vectorized environments"

        # Compute the no op vertors
        self.observation_no_op = self.env_to_wrap.wrap_observation(
            self.env_to_wrap.env_to_wrap.observation_space.no_op()
        )["vector"]
        self.action_no_op = self.env_to_wrap.wrap_action(
            self.env_to_wrap.env_to_wrap.action_space.no_op()
        )["vector"]
    ...
    def _get_obfuscator(obfuscator_dir: Union[str, Path]):
        ...
        obs_enc, obs_dec = np.load(
            os.path.join(obfuscator_dir, OBSERVATION_OBFUSCATOR_FILE_NAME),
            allow_pickle=True,
        )["arr_0"]
    ...
    def create_observation_space(self):
        ...
        obs_space.spaces["vector"] = spaces.Box(
            low=-1.2, high=1.2, shape=[self.obf_vector_len]
        )
        return obs_space

    def create_action_space(self):
        act_space = copy.deepcopy(self.env_to_wrap.action_space)
        act_space.spaces["vector"] = spaces.Box(
            low=-1.05, high=1.05, shape=[self.obf_vector_len]
        )
        return act_space
