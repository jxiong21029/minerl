...
def union_spaces(
    hdls_1: List[Handler], hdls_2: List[Handler]
) -> List[MineRLSpace]:
...
def flatten_spaces(
    hdls: List[Handler],
) -> Tuple[list, List[Tuple[str, MineRLSpace]]]:
    return [
        hdl.space.flattened for hdl in hdls if hdl.space.is_flattenable()
    ], [
        (hdl.to_string(), hdl.space)
        for hdl in hdls
        if not hdl.space.is_flattenable()
    ]
