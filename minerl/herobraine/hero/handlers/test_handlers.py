...
from minerl.herobraine.hero.handlers.agent.observations.compass import (
    CompassObservation,
)
from minerl.herobraine.hero.handlers.agent.observations.inventory import (
    FlatInventoryObservation,
)
from minerl.herobraine.hero.handlers.agent.observations.equipped_item import (
    _TypeObservation,
)
...
def test_merge_item_list_command_actions():
    ...
    assert TestItemListCommandAction(
        ["none", "A", "B", "C", "D"], _other="none"
    ) | TestItemListCommandAction(
        ["none", "E", "F"], _other="none"
    ) == TestItemListCommandAction(
...
def test_merge_type_observation():
    ...
    type_obs_result = _TypeObservation(
        "test",
...
def test_merge_flat_inventory_observation():
    assert FlatInventoryObservation(
        ["stone", "sandstone", "lumber", "wood", "iron_bar"]
    ) | FlatInventoryObservation(
        ["ice", "ice", "ice", "ice", "ice", "water"]
    ) == FlatInventoryObservation(
        ["stone", "sandstone", "lumber", "wood", "iron_bar", "ice", "water"]
    )
