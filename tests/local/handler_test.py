...
def test_wrapped_env(
    environment="MineRLObtainTest-v0", wrapped_env="MineRLObtainTestVector-v0"
):
        ...
        for action in gen_obtain_debug_actions(env):
            for key, value in action.items():
                if (
                    ...
                    and value in reward_dict
                    and key not in ["equip"]
                ):
                    print(
                        "Action of {}:{} if successful gets {}".format(
                            key, value, reward_dict[value]
                        )
                    )
